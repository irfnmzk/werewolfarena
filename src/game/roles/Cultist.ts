...
export default class Cultist extends Role {
  ...
  public eventAnnouncement() {
    this.game.channel.sendWithText(
      this.userId,
      this.game.localeService.t('role.cultist.announcement')
    );
  }
  ...
  public eventNight() {
    ...
    const message = this.messageGenerator.cultistSelection(target);
    if (target.length > 0) {
